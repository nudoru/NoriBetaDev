package com.nudoru.sheet {
	...
	public class ImageLoader extends Sprite {
		...
		public function ImageLoader(n:String, t:Sprite, initObj:Object):void {
			...
			// move the border to the top for this style
			if (_BorderStyle == "jframe") _Container.addChild(border);
		...
		protected function applyBorder(border:Sprite):void
		{
			var bdrHeight:int = _Height + (_Border * 2);
			...
			} else if (_BorderStyle == "jframe") {
				var thinline:Sprite = new Sprite();
				var thinkline:Sprite = new Sprite();
				thinline.graphics.lineStyle(1, _BOutlineColor, 1, true);
				thinline.graphics.drawRect( 0, 0, _Width + (_Border * 2), _Height + (_Border * 2));
				thinkline.graphics.lineStyle(_Border * 2, _BLightColor, 1, true, "normal", CapsStyle.SQUARE, JointStyle.MITER);
				thinkline.graphics.drawRect( _Border * 2, _Border * 2, _Width - (_Border * 2), _Height - (_Border * 2));
				thinkline.blendMode = BlendMode.MULTIPLY;
				border.addChild(thinkline);
				border.addChild(thinline);
			} else {
